...
class EditElderSayingsScreen extends ConsumerStatefulWidget {
  const EditElderSayingsScreen({super.key});

  @override
  ConsumerState<EditElderSayingsScreen> createState() =>
      _EditElderSayingsScreenState();
}

class _EditElderSayingsScreenState
    extends ConsumerState<EditElderSayingsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final content = await service.getElderSayings();
    if (mounted) {
      setState(() {
  ...
  Future<void> _save() async {
    ...
    try {
      final service = ref.read(contentServiceProvider);
      await service.updateElderSayings(
        ElderSayingsContent(
          sayings: _sayings,
  ...
  void _addSaying() {
    ...
    _showEditDialog(
      ElderSaying(
        id: 'elder_${DateTime.now().millisecondsSinceEpoch}',
        name: const {'en': '', 'ne': ''},
        title: const {'en': '', 'ne': ''},
        saying: const {'en': '', 'ne': ''},
        order: nextOrder,
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          content: Text(
              'Remove ${_sayings[index].localizedName('en')}\'s saying?'),
          actions: [
            TextButton(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(
                        ...
                        hintText: 'जस्तै: परिवारका ज्येष्ठ'),
                  ),
                  const SizedBox(height: 8),
                  TextFormField(
                    controller: sayingEnCtrl,
                    decoration: const InputDecoration(
                      labelText: 'Saying (English)',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${l10n.editContent} — ${l10n.elderSayings}'),
        actions: [
          if (_isSaving)
            const Padding(
