...
class EditBideshSamitiScreen extends ConsumerStatefulWidget {
  const EditBideshSamitiScreen({super.key});

  @override
  ConsumerState<EditBideshSamitiScreen> createState() =>
      _EditBideshSamitiScreenState();
}

class _EditBideshSamitiScreenState
    extends ConsumerState<EditBideshSamitiScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final content = await service.getBideshSamiti();
    if (mounted) {
      setState(() {
  ...
  Future<bool> _persistChanges({
    ...
    try {
      final service = ref.read(contentServiceProvider);
      await service.updateBideshSamiti(
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(title: Text(l10n.bideshSamiti)),
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${l10n.editContent} — ${l10n.bideshSamiti}'),
        actions: [
          if (_isSaving)
            const Padding(
